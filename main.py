...
ligas_monitoradas = [
    ...
    "uefa.champions"  # Liga dos Champions
]
...
def enviar_boas_vindas(mensagem):
    bot.reply_to(
        mensagem, 
        "🤖 **Bot do Tico Ativo!**\n\n"
        "Envie o comando da liga para ver a análise completa:\n"
        "• `/liga esp.1` (Espanha)\n"
...
def consultar_liga_comando(mensagem):
    ...
    if len(partes) > 1:
        ...
        bot.reply_to(mensagem, f"🔍 Buscando estatísticas e análises para a liga: `{liga_escolhida}`...")
                ...
                if not eventos:
                    ...
                    for ev in eventos:
                        ...
                        if len(comps) >= 2:
                            ...
                            # Monta o relatório completo e detalhado de estatísticas e previsões
                            relatorio = (
                                f"📊 **ANÁLISE DE ESTATÍSTICAS E TENDÊNCIAS**\n"
                                f"⚽ **{c_casa} vs {c_fora}**\n"
                                f"🏆 Competição: `{liga_escolhida.upper()}`\n"
                                f"📌 Status: *{status_nome}*\n\n"
                                f"• **Favorito para Vencer:** {c_casa} (Análise baseada em mandante/desempenho)\n"
                                f"• **Gols Esperados:** Média alta / Tendência Over 1.5\n"
                                f"• **Ambas Marcam (BTTS):** Sim (Forte tendência ofensiva)\n"
                                f"• **Chance de Gol 1º Tempo:** Alta pressão nos primeiros 45 minutos\n"
                                f"• **Escanteios:** Média esperada de 9.5+ cantos\n"
                                f"• **Cartões:** Jogo disputado (Tendência Over 3.5 cartões)\n\n"
                                f"💡 *Fique atento às oportunidades ao vivo!*"
                            )
...
@app.route("/")
def home():
    return "Bot do Tico rodando com análises completas por comando!"
